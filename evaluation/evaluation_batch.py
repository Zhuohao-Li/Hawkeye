...
# Evaluation logic
######################################
def evaluate_model(model_path, test_file, eval_mode="joint", is_local=False, api_key="", batch_size=1):
    ...
    print(f"Test dataloader contains {len(test_dataloader)} batches.")

    results = {"correct": 0, "total": 0, "cot_lengths": [], "answer_lengths": [], "predictions": []}
    for batch in tqdm(test_dataloader, desc="Evaluating samples"):
        ...
        cots = generate_text_standard(eval_model, tokenizer_big, prompts, max_new_tokens=1024, temperature=0.7, top_p=0.9)
        cots = [clean_cot(cot, prompt) for cot, prompt in zip(cots, batch["prompt"])]
        results["cot_lengths"].extend([len(cot.split()) for cot in cots])

        prompt_bs = [f"{cot}\n This is a chain-of-thought that helps you answer. So the answer is:" 
                    for cot in cots]
        answer_bs = generate_text_standard(eval_model_small, tokenizer_small, prompt_bs, max_new_tokens=512, temperature=0.7, top_p=0.9)
        results["answer_lengths"].extend([len(answer.split()) for answer in answer_bs])
        
        for cot, answer_b, ground_truth in zip(cots, answer_bs, ground_truths):
...
#python evaluation_batch.py --model_path Qwen/Qwen1.5-7B --test_file test_data.jsonl --eval_mode joint --batch_size 8

# 使用本地模型进行评估
#python evaluation_batch.py --model_path ./checkpoints/checkpoint-50 --test_file ./dataset/test_dataset.jsonl --eval_mode single --is_local --batch_size 8
